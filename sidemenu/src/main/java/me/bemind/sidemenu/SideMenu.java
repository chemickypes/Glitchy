 ...
 * pattern instead. implements this pattern.)</p>
 *
...
public class SideMenu extends ViewGroup {
    ...
    /**
     * the child view that appears bottom slideableView
     */
    ...
    public interface PanelSlideListener {
         ...
         */
        public void onPanelSlide(View panel,View bottomPanel, float slideOffset);
        /**
    ...
    public static abstract class MenuPanelSlideListener implements PanelSlideListener {
        ...
        private float offsetDiff = OFFSET_DIFF;
        private float menuStartOffset = MENU_START_OFFSET;

        @Override
        public void onPanelSlide(View panel,View bottomPanel, float slideOffset) {
            float scaleFactor = 1 - (offsetDiff *slideOffset);
//        float scaleFactor2 = (1-OFFSET_DIFF)+(OFFSET_DIFF*v);
            float scaleFactor2 = menuStartOffset - slideOffset;
...
//                Log.d(TAG,"SlidingPanel child count"+vg.getChildCount());
    ...
    void dispatchOnPanelSlide(View panel,View bottomPanel) {
    ...
    private void onPanelDragged(int newLeft) {
        ...
        dispatchOnPanelSlide(mSlideableView,mBottomView);
    }
    ...
    private class DragHelperCallback extends ViewDragHelper.Callback {
        ...
        public void onViewDragStateChanged(int state) {
            if (mDragHelper.getViewDragState() == ViewDragHelper.STATE_IDLE) {
                if (mSlideOffset == 0) {
                    updateObscuredViewsVisibility(mSlideableView);
                    dispatchOnPanelClosed(mSlideableView,mBottomView);
                    mPreservedOpenState = false;
                } else {
                    dispatchOnPanelOpened(mSlideableView,mBottomView);
                    mPreservedOpenState = true;
                }
            }
        }
    ...
    interface SlidingPanelLayoutImpl {
        void invalidateChildRegion(SideMenu parent, View child);
    }

    static class SlidingPanelLayoutImplBase implements SlidingPanelLayoutImpl {
        public void invalidateChildRegion(SideMenu parent, View child) {
    ...
    private class DisableLayerRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (mChildView.getParent() == SideMenu.this) {
